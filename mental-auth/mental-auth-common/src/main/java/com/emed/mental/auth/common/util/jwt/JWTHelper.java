package com.emed.mental.auth.common.util.jwt;

import com.emed.mental.auth.common.constant.CommonConstants;
import com.emed.mental.auth.common.util.StringHelper;
...
public class JWTHelper {
    ...
    public static String generateToken(IJWTInfo jwtInfo, byte priKey[], int expire) throws Exception {
        String compactJws = Jwts.builder()
                .setSubject(jwtInfo.getUniqueName()) //主题
                .claim(CommonConstants.JWT_KEY_USER_ID, jwtInfo.getId()) // 自定义属性
                .claim(CommonConstants.JWT_KEY_NAME, jwtInfo.getName())  //自定义属性
                .setExpiration(DateTime.now().plusSeconds(expire).toDate()) //过期时间
                .signWith(SignatureAlgorithm.RS256, rsaKeyHelper.getPrivateKey(priKey)) // 签名算法以及密匙
                .compact();
        return compactJws;
    }
